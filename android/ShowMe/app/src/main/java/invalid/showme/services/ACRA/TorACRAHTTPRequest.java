...
//From https://github.com/ACRA/acra/blob/master/src/main/java/org/acra/util/HttpRequest.java
public final class TorACRAHTTPRequest {
    private static String TAG = "TorACRAHTTPRequest";
    ...
    public TorACRAHTTPRequest(@NonNull ACRAConfiguration config) {
        this.config = config;
    }
     ...
     */
    public void send(@NonNull Context context, @NonNull URL url, @NonNull Method method, @NonNull String content, @NonNull Type type) throws IOException, InterruptedException {

        HttpURLConnection urlConnection = null;
        if(PreferenceManager.getDefaultSharedPreferences(context).getBoolean("pref_usetor", false)) {
            urlConnection = (HttpURLConnection) url.openConnection(ServerInterface.torProxy);
            if(!OrbotHelper.isOrbotInstalled(context)) {
                //TODO: Runs in background process??
        ...
        if (ACRA.DEV_LOGGING) ACRA.log.d(TAG, "Sending request to " + url);
        if (ACRA.DEV_LOGGING) ACRA.log.d(TAG, "Http " + method.name() + " content : ");
        if (ACRA.DEV_LOGGING) ACRA.log.d(TAG, content);

        final int responseCode = urlConnection.getResponseCode();
        if (ACRA.DEV_LOGGING)
            ACRA.log.d(TAG, "Request response : " + responseCode + " : " + urlConnection.getResponseMessage());
        if ((responseCode >= HTTP_SUCCESS) && (responseCode < HTTP_REDIRECT)) {
            // All is good
            ACRA.log.i(TAG, "Request received by server");
            ...
            ACRA.log.w(TAG, "401: Login validation error on server - request will be discarded");
            ...
            ACRA.log.w(TAG, "403: Data validation error on server - request will be discarded");
            ...
            //This probably means that nothing can be sent with this configuration, maybe ACRA should disable itself after it?
            ACRA.log.w(TAG, "405: Server rejected Http " + method + " - request will be discarded");
        } else if (responseCode == HTTP_CONFLICT) {
            // 409 means that the report has been received already. So we can discard it.
            ACRA.log.w(TAG, "409: Server has already received this post - request will be discarded");
        } else if ((responseCode >= HTTP_CLIENT_ERROR) && (responseCode < MAX_HTTP_CODE)) {
            ACRA.log.w(TAG, "Could not send ACRA Post responseCode=" + responseCode + " message=" + urlConnection.getResponseMessage());
            throw new IOException("Host returned error code " + responseCode);
        } else {
            ACRA.log.w(TAG, "Could not send ACRA Post - request will be discarded. responseCode=" + responseCode + " message=" + urlConnection.getResponseMessage());
        }
